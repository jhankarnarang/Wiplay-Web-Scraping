...
def login():
	if user_name.get()=="" or password.get()=="":
		...
		con = pymysql.connect(host="localhost",user="root",password="Shub@8877240694",database="docterapp")
		cur = con.cursor()

		cur.execute("select * from user_data where username=%s and password = %s",(user_name.get(),password.get()))
		row = cur.fetchone()
...
def deshboard():
	...
	Docter_box['values']=('Dr.Mohan','Dr. Rohan','Dr.Shyam', 'Dr. Andy')
	...
	def action():
		if first_name.get()=="" or last_name.get()=="" or age.get()=="" or city.get()=="" or add.get()=="" or user_name.get()=="" or password.get()=="" or very_pass.get()=="":
			...
			if row!=None:
				...
				cur.execute("insert into user_data(first_name,last_name,age,gender,city,address,username,password) values(%s,%s,%s,%s,%s,%s,%s,%s)",
					(
					first_name.get(),
					last_name.get(),
					age.get(),
